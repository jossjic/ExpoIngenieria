<?php
	...
	if ( !empty($_POST)) {
		...
		$sql = "DELETE FROM CATEGORIA WHERE ca_id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
		Database::disconnect();
	    ...
	    <div class="container">
	    	<div class="span10 offset1">
			    ...
			    <form class="form-horizontal" action="deleteCategoria.php" method="post">
					...
					<p class="alert alert-error">Estas seguro que quieres eliminar esta categoria ?</p>
					<div class="form-actions">
						<button type="submit" class="btn btn-danger">Si</button>
						<a class="btn" href="CategoriaCRUD.php">No</a>
					</div>
				</form>
			</div>
	  </div> <!-- /container -->
